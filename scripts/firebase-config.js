...
const CANONICAL_REDIRECT_URL = "https://school.ourstuff.space";
...
function deriveRedirectUrl() {
	try {
		...
		if (
			ALLOWED_REDIRECT_ORIGINS.has(candidate) ||
		...
		console.warn(
			`Redirect origin not whitelisted: ${origin}. Falling back to canonical domain.`,
		);
	} catch (e) {
		console.warn(
			"Could not derive redirect URL; falling back to canonical domain.",
			e,
		);
	}
